...
class InteractiveRecord
    ...
    def values_for_insert
        vals = []
        self.class.column_names.each {|col_name| vals << "'#{send(col_name)}'" unless send(col_name).nil?}
        vals.join(", ")
    end
    ...
    def self.find_by(attribute_hash)
        ...
        DB[:conn].execute("SELECT * FROM #{self.table_name} WHERE #{attribute_hash.keys.first} = #{form_value}")
    end
end
